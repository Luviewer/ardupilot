...
class AC_AttitudeControl_Multi_Tilt : public AC_AttitudeControl_Multi {
public:
    AC_AttitudeControl_Multi_Tilt(AP_AHRS_View& ahrs, const AP_MultiCopter& aparm, AP_MotorsMulticopter& motors)
        : AC_AttitudeControl_Multi(ahrs, aparm, motors)
    {

        if (_singleton != nullptr) {
            AP_HAL::panic("Can only be one AC_AttitudeControl_Multi_Tilt");
        }
        _singleton = this;
    }

    static AC_AttitudeControl_Multi_Tilt* get_singleton()
    {
        return _singleton;
    }
