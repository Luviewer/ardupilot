...
public:
    ...
    static const struct AP_Param::GroupInfo* backend_var_info[AP_HEXRUPED_GAIT_COUNT];
    ...
    const AP_HexRuped_SYS_Params&     get_sys_params() const { return _sys_params; }
    const AP_HexRuped_CHANNEL_Params& get_channel_params() const { return _channel_params; }
    AP_HexRuped_CTRL_Params&          get_ctrl_params() { return _ctrl_params; }
...
private:
    // 状态结构体定义
    struct HexRuped_State {
        uint32_t                          last_time_ms;
        uint8_t                           instance;
        const struct AP_Param::GroupInfo* var_info;
    } _state[AP_HEXRUPED_GAIT_COUNT];
    ...
    // 后端管理
    AP_HexRuped_Backend* _backend;                                // 当前活跃的后端
    AP_HexRuped_Backend* _gait_backends[AP_HEXRUPED_GAIT_COUNT]; // 所有的步态后端
    ...
    // 参数组
    AP_HexRuped_SYS_Params     _sys_params;                       // 系统参数
    AP_HexRuped_Params         _leg_params[AP_HEXRUPED_LEG_ALL]; // 腿部参数
