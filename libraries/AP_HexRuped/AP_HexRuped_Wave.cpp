#include "AP_HexRuped_Wave.h"
#include "AP_HexRuped.h"
#include <AP_HAL/AP_HAL.h>

#define SPEED_HZ_DEFAULT        AP_HEXRUPED_SPEED_HZ_DEFAULT // 默认步态频率（Hz）
#define GAIT_STEP_TOTAL_DEFAULT AP_HEXRUPED_STEP_TOTAL_DEFAULT // 默认步态总步数
...
// 构造函数
AP_HexRuped_Wave::AP_HexRuped_Wave(AP_HexRuped& frontend, AP_HexRuped::HexRuped_State& state, AP_AHRS_View& ahrs, AP_Motors& motors)
...
// 步态初始化
void AP_HexRuped_Wave::gait_init()
{
    gcs().send_text(MAV_SEVERITY_INFO, "AP_HexRuped_Wave init - Single Leg Swing");

    // 六足波浪步态每次仅摆动一条腿，摆动相占完整周期的1/6。
    gait_lift_divisor = AP_HEXRUPED_LEG_ALL;
    if (gait_step_total.get() < gait_lift_divisor) {
        gait_step_total.set(gait_lift_divisor);
    }
    const int16_t step_total = gait_step_total.get();

    static const uint8_t wave_order[AP_HEXRUPED_LEG_ALL] = {
        AP_HEXRUPED_LEG_RF,
        AP_HEXRUPED_LEG_RM,
        AP_HEXRUPED_LEG_RB,
        AP_HEXRUPED_LEG_LB,
...
// 获取活跃腿索引
uint8_t AP_HexRuped_Wave::get_active_leg_index()
{
    static const uint8_t wave_order[AP_HEXRUPED_LEG_ALL] = {
        AP_HEXRUPED_LEG_RF,
        AP_HEXRUPED_LEG_RM,
    ...
    const int16_t step_total = gait_step_total.get();
    if (step_total <= 0) {
        return AP_HEXRUPED_LEG_RF;
    }
    const uint16_t cycle_position = gait_step_now % step_total;
    const uint8_t phase_index = MIN((cycle_position * AP_HEXRUPED_LEG_ALL) / step_total,
                                    AP_HEXRUPED_LEG_ALL - 1);
    return wave_order[phase_index];
}

// 更新腿部运动
void AP_HexRuped_Wave::update_leg()
{
...
void AP_HexRuped_Wave::trajectory_generation(uint8_t leg_index)
{
    // 轨迹模式决策：根据参数选择轨迹生成算法
...
// 波浪步态特点：六条腿依次摆动，其余五条腿保持支撑。
void AP_HexRuped_Wave::generate_cycloid_trajectory(uint8_t leg_index)
{
    ...
    const float swing_ratio = 1.0f / (float)gait_lift_divisor; // 单腿摆动占 1/6 周期
...
// 波浪步态特点：六条腿依次摆动，其余五条腿保持支撑。
void AP_HexRuped_Wave::generate_bezier_trajectory(uint8_t leg_index)
{
...
// 三次贝塞尔曲线计算
Vector3f AP_HexRuped_Wave::cubic_bezier_trajectory(float t, const Vector3f& p0, const Vector3f& p1, const Vector3f& p2, const Vector3f& p3)
{
...
// 主逆运动学
void AP_HexRuped_Wave::main_inverse_kinematics()
{
    AP_HexRuped_Backend::main_inverse_kinematics();
}
