...
{
    gcs().send_text(MAV_SEVERITY_INFO, "AP_QuadRuped_Wave_COG init - Single Leg Swing");

    refresh_phase_offsets();
}

void AP_QuadRuped_Wave_COG::refresh_phase_offsets()
{
...
void AP_QuadRuped_Wave_COG::trajectory_generation(uint8_t leg_index)
{
    // 轨迹模式决策：根据参数选择轨迹生成算法
...
// 偏航轨迹生成
void AP_QuadRuped_Wave_COG::yaw_trajectory_generation(uint8_t leg_index)
{
    int16_t delta_step = gait_step_now - gait_step_leg_start[leg_index];
...
void AP_QuadRuped_Wave_COG::generate_cycloid_trajectory(uint8_t leg_index)
{
    // 步态相位计算：与贝塞尔曲线轨迹相同的相位处理逻辑
    // 这种统一设计保证了不同轨迹算法之间的相位一致性
...
// 波浪步态特点：单腿依次运动，需要配合重心偏移保持稳定
void AP_QuadRuped_Wave_COG::generate_bezier_trajectory(uint8_t leg_index)
{
...
// 三次贝塞尔曲线计算
Vector3f AP_QuadRuped_Wave_COG::cubic_bezier_trajectory(float t, const Vector3f& p0, const Vector3f& p1, const Vector3f& p2, const Vector3f& p3)
{
...
// 平衡控制器
void AP_QuadRuped_Wave_COG::balance_controller()
{
...
// 相位缓存更新
void AP_QuadRuped_Wave_COG::update_phase_cache(float angle, uint8_t leg_index)
{
...
// 获取缓存的sin值
float AP_QuadRuped_Wave_COG::get_cached_sin(float angle, uint8_t leg_index)
{
...
// 获取缓存的cos值
float AP_QuadRuped_Wave_COG::get_cached_cos(float angle, uint8_t leg_index)
{
    update_phase_cache(angle, leg_index);
