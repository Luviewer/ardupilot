...
此脚本为共轴三旋翼（6个电机，3个共轴对）配置动态混控，实现三个电机的倾转控制，用于X轴（pitch）方向自由度解耦。

**重要**：倾转角度通过pitch控制输出计算，而非从舵机读取（舵机一般无法读取当前角度）。
...
### 倾转机构
- **三个电机均可倾转**：前左、前右、后置电机都可通过舵机实现倾转
- 倾转角度范围：-45° 到 +45°（可配置）
- 倾转舵机连接到飞控的舵机输出通道
- **倾转角度计算**：根据pitch控制输出自动计算目标倾转角度
...
local TILT_SERVO_REAR = 11     -- 后置倾转舵机功能号（新增）
local TILT_PITCH_GAIN = 1.0    -- Pitch控制增益
local REAR_TILT_RATIO = 0.0   -- 后置倾转比例（0.0=不倾转，1.0=与前电机相同）
...
3. **舵机功能分配**: 配置倾转舵机
   ...
   SERVO11_FUNCTION = 72 (或自定义功能号，需与脚本中TILT_SERVO_REAR一致，如果使用后置倾转)
   ```
...
1. **Roll控制（动态）**: 
   - 前左电机：`roll_factor = -0.5 × cos(倾转角度)`
   - 前右电机：`roll_factor = +0.5 × cos(倾转角度)`
   - 后电机：零roll因子（ly=0，不贡献roll）

2. **Pitch控制（动态）**:
   - 前左电机：`pitch_factor = sin(倾转角度) × 增益 × 0.5`
   - 前右电机：`pitch_factor = -sin(倾转角度) × 增益 × 0.5`
   - 后电机：`pitch_factor = cos(倾转角度) × 0.5`（如果后置倾转，否则固定0.5）

3. **Yaw控制**:
   - 所有共轴对的yaw因子为0（上下桨旋转方向相反，不产生净yaw力矩）

4. **Throttle控制（动态）**:
   - 所有电机throttle因子受倾转影响：`throttle_factor = cos(倾转角度)`
   - 倾转会导致升力损失，通过cos项补偿

### 倾转角度计算

**重要**：脚本通过pitch控制输出计算目标倾转角度，而非从舵机读取。
...
**倾转策略**：
- **前左和前右**：对称倾转（相反方向），根据pitch需求自动计算
- **后置**：根据 `REAR_TILT_RATIO` 决定是否倾转（默认0.0，不倾转）

倾转角度通过 `sin()` 和 `cos()` 函数转换为混控因子，实现非线性控制。
...
- 舵机通道未正确连接
- PWM范围设置不正确
- pitch控制输出为0（无pitch需求时倾转角度为0）

**解决方法**:
- 检查 `SERVOx_FUNCTION` 参数
- 验证舵机物理连接
- 检查PWM范围（1000-2000）
- 检查pitch控制输出是否正常（通过GCS查看）
